...
namespace Rendering
{
	...
	const GLfloat CubeDemo::RotationRate = M_PI;

	CubeDemo::CubeDemo(Game& game, Camera& camera) :
		DrawableGameComponent(game, camera), mVertexArrayObject(0), mVertexBuffer(0),
		mIndexBuffer(0), mWorldViewProjectionLocation(-1), mIndexCount(0), mWorldMatrix(Eigen::Matrix4f::Identity())
	{
	}
	...
	void CubeDemo::Initialize()
	{
		...
		*/
		VertexPositionColor vertices[] =
		{
			VertexPositionColor(Vector4f(-1.0f, +1.0f, -1.0f, 1.0f), ColorHelper::Green),
			VertexPositionColor(Vector4f(+1.0f, +1.0f, -1.0f, 1.0f), ColorHelper::Yellow),
			VertexPositionColor(Vector4f(+1.0f, +1.0f, +1.0f, 1.0f), ColorHelper::White),
			VertexPositionColor(Vector4f(-1.0f, +1.0f, +1.0f, 1.0f), ColorHelper::BlueGreen),

			VertexPositionColor(Vector4f(-1.0f, -1.0f, +1.0f, 1.0f), ColorHelper::Blue),
			VertexPositionColor(Vector4f(+1.0f, -1.0f, +1.0f, 1.0f), ColorHelper::Purple),
			VertexPositionColor(Vector4f(+1.0f, -1.0f, -1.0f, 1.0f), ColorHelper::Red),
			VertexPositionColor(Vector4f(-1.0f, -1.0f, -1.0f, 1.0f), ColorHelper::Black)
		};
	...
	void CubeDemo::Update(const GameTime& gameTime)
	{
		...
		mWorldMatrix = Affine3f(AngleAxisf(angle, Vector3Helper::Up)).matrix();
	}
