...
namespace Library
{
		...
		mModel(model), mMaterial(nullptr), mName(mesh.mName.C_Str()), mFaceCount(0)
    {
        ...
        for (uint32_t i = 0; i < mesh.mNumVertices; i++)
        {
			...
			mVertices.push_back(Eigen::Vector3f(v.x, v.y, v.z));
        }
        ...
        if (mesh.HasNormals())
        {
            ...
            for (uint32_t i = 0; i < mesh.mNumVertices; i++)
            {
				...
				mNormals.push_back(Eigen::Vector3f(n.x, n.y, n.z));
            }
        }
        ...
        if (mesh.HasTangentsAndBitangents())
        {
            ...
            for (uint32_t i = 0; i < mesh.mNumVertices; i++)
            {
				...
				mTangents.push_back(Eigen::Vector3f(t.x, t.y, t.z));
                
				aiVector3D b = mesh.mBitangents[i];
				//mBiNormals.push_back(glm::vec3(b.x, b.y, b.z));
				mBiNormals.push_back(Eigen::Vector3f(b.x, b.y, b.z));
            }
        }
        ...
        for (uint32_t i = 0; i < uvChannelCount; i++)
        {
			...
			vector<Eigen::Vector3f>* textureCoordinates = new vector<Eigen::Vector3f>();
            ...
            for (uint32_t j = 0; j < mesh.mNumVertices; j++)
            {
				...
				textureCoordinates->push_back(Eigen::Vector3f(uv.x, uv.y, uv.z));
            }
        }
        ...
        for (uint32_t i = 0; i < colorChannelCount; i++)
        {
			...
			vector<Eigen::Vector4f>* vertexColors = new vector<Eigen::Vector4f>();
            ...
            for (uint32_t j = 0; j < mesh.mNumVertices; j++)
            {
				...
				vertexColors->push_back(Eigen::Vector4f(c.r, c.g, c.b, c.a));
            }
        }
    ...
    Mesh::~Mesh()
    {
		...
		for (vector<Eigen::Vector3f>* textureCoordinates : mTextureCoordinates)
        {
            delete textureCoordinates;
        }

        //for (vector<glm::vec4>* vertexColors : mVertexColors)
		for (vector<Eigen::Vector4f>* vertexColors : mVertexColors)
        {
            delete vertexColors;
        }
    }
	...
	const vector<Eigen::Vector3f>& Mesh::Vertices() const
    {
        return mVertices;
    }

    //const vector<glm::vec3>& Mesh::Normals() const
	const vector<Eigen::Vector3f>& Mesh::Normals() const
    {
        return mNormals;
    }

    //const vector<glm::vec3>& Mesh::Tangents() const
	const vector<Eigen::Vector3f>& Mesh::Tangents() const
    {
        return mTangents;
    }

    //const vector<glm::vec3>& Mesh::BiNormals() const
	const vector<Eigen::Vector3f>& Mesh::BiNormals() const
    {
        return mBiNormals;
    }

    //const vector<vector<glm::vec3>*>& Mesh::TextureCoordinates() const
	const vector<vector<Eigen::Vector3f>*>& Mesh::TextureCoordinates() const
    {
        return mTextureCoordinates;
    }

    //const vector<vector<glm::vec4>*>& Mesh::VertexColors() const
	const vector<vector<Eigen::Vector4f>*>& Mesh::VertexColors() const
    {
