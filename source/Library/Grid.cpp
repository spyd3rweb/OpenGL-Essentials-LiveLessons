...
namespace Library
{
	...
	const Vector4f Grid::DefaultColor = Vector4f(0.961f, 0.871f, 0.702f, 1.0f);

	//Grid::Grid(Game& game, Camera& camera, GLuint size, GLuint scale, const vec4& color) :
	Grid::Grid(Game& game, Camera& camera, GLuint size, GLuint scale, const Vector4f& color) :
		...
		mVertexCount(0), mWorldMatrix(Eigen::Matrix4f::Identity())
	{
	}
	...
	const Vector4f& Grid::Color() const
	{
		return mColor;
	}
	...
	*/
	void Grid::SetPosition(const Vector3f& position)
	{
		mPosition = position;
		mWorldMatrix = Affine3f(Translation3f(mPosition)).matrix();
	}
	...
	void Grid::SetColor(const Vector4f& color)
	{
		mColor = color;
		InitializeGrid();
	}
	...
	void Grid::InitializeGrid()
	{
        ...
        for (unsigned int i = 0, j = 0; i < mSize + 1; i++, j = 4 * i)
        {
			...
			*/
			vertices[j] = VertexPositionColor(Vector4f(position, 0.0f, maxPosition, 1.0f), mColor);
            vertices[j + 1] = VertexPositionColor(Vector4f(position, 0.0f, -maxPosition, 1.0f), mColor);
			...
			*/
            ...
            vertices[j + 2] = VertexPositionColor(Vector4f(maxPosition, 0.0f, position, 1.0f), mColor);
            vertices[j + 3] = VertexPositionColor(Vector4f(-maxPosition, 0.0f, position, 1.0f), mColor);
        }
