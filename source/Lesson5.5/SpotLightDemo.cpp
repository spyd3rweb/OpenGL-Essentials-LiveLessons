...
namespace Rendering
{
	...
	const Vector2f SpotLightDemo::LightRotationRate = Vector2f(2.0f * M_PI,2.0f * M_PI);
	...
	SpotLightDemo::SpotLightDemo(Game& game, Camera& camera) :
		...
		mIndexBuffer(0), mWorldMatrix(Matrix4f::Identity()), mIndexCount(0), mColorTexture(0),
	...
	void SpotLightDemo::Initialize()
	{
		...
		mProxyModel->ApplyRotation(Affine3f(AngleAxisf(M_PI_2, Vector3Helper::Right)).matrix());

		//mWorldMatrix = scale(mat4(), vec3(5.0f));
		mWorldMatrix = Affine3f(Eigen::UniformScaling<float>(5.0f)).matrix();
	}
	...
	void SpotLightDemo::UpdateSpotLight(const GameTime& gameTime)
	{
		...
		if (glfwGetKey(mGame->Window(), GLFW_KEY_HOME) && directionalIntensity < 1.0f)
		{
			...
			mSpotLight->SetColor(Vector4f(directionalIntensity, directionalIntensity, directionalIntensity, 1.0f));
		}
		...
		Vector3f movementAmount = Vector3Helper::Zero;
		...
		Vector3f movement = movementAmount * LightMovementRate * elapsedTime;
		...
		if (rotationAmount.y() != 0)
		{
			...
			lightRotationMatrix = lightRotationMatrix * Affine3f(AngleAxisf(rotationAmount.y(), mSpotLight->Right()));
		}

		if (rotationAmount.x() != 0.0f || rotationAmount.y() != 0.0f)
		{
			mSpotLight->ApplyRotation(lightRotationMatrix.matrix());
