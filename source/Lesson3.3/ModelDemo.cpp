...
namespace Rendering
{
	...
	void ModelDemo::CreateVertexBuffer(const Mesh& mesh, GLuint& vertexBuffer)
	{
		...
		if (mesh.VertexColors().size() > 0)
		{
			...
			vector<Vector4f>* vertexColors = mesh.VertexColors().at(0);
			assert(vertexColors->size() == sourceVertices.size());

			for (size_t i = 0; i < sourceVertices.size(); i++)
			{
				/*
				onst vec3& position = sourceVertices.at(i);
				...
				*/
				...
				const Vector4f& color = vertexColors->at(i);
				vertices.push_back(VertexPositionColor(Vector4f(position.x(), position.y(), position.z(), 1.0f), color));
			}
		}
		else
		{
			for (size_t i = 0; i < sourceVertices.size(); i++)
			{
				...
				*/
				const Vector3f& position = sourceVertices.at(i);
				const Vector4f& color = ColorHelper::RandomColor();
				vertices.push_back(VertexPositionColor(Vector4f(position.x(), position.y(), position.z(), 1.0f), color));
			}
		}
