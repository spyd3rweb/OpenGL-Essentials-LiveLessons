...
namespace Rendering
{
	...
	FilteringModesDemo::FilteringModesDemo(Game& game, Camera& camera) :
		...
		mActiveFilteringMode(FilteringMode::Point), mKeyboardHandler(nullptr),mWorldMatrix(Eigen::Matrix4f::Identity())
	{
	}
	...
	void FilteringModesDemo::Initialize()
	{
		...
		VertexPositionTexture vertices[] =
		{
			...
			*/
			VertexPositionTexture(Vector4f(-halfSize, 1.0f, 0.0, 1.0f), Vector2f(0.0f, 1.0f)),
			VertexPositionTexture(Vector4f(-halfSize, size + 1.0f, 0.0f, 1.0f), Vector2f(0.0f, 0.0f)),
			VertexPositionTexture(Vector4f(halfSize, size + 1.0f, 0.0f, 1.0f), Vector2f(1.0f, 0.0f)),
			VertexPositionTexture(Vector4f(halfSize, 1.0f, 0.0f, 1.0f), Vector2f(1.0f, 1.0f))
		};
