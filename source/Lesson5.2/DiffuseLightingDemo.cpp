...
namespace Rendering
{
	...
	const Vector2f DiffuseLightingDemo::LightRotationRate = Vector2f(2.0f * M_PI, 2.0f * M_PI);
	...
	DiffuseLightingDemo::DiffuseLightingDemo(Game& game, Camera& camera) :
		...
		mLightColorLocation(-1), mLightDirectionLocation(-1), mIndexCount(0), mColorTexture(0), mWorldMatrix(Eigen::Matrix4f::Identity())
	{
	}
	...
	void DiffuseLightingDemo::Initialize()
	{
		...
		mProxyModel->ApplyRotation(Affine3f(AngleAxisf(MatrixHelper::Radians(90.0f), Vector3Helper::Up)).matrix());
	}
	...
	void DiffuseLightingDemo::Draw(const GameTime& gameTime)
	{
		...
		*/
		...
		glUniformMatrix4fv(mWorldViewProjectionLocation, 1, GL_FALSE, wvp.data());
		glUniformMatrix4fv(mWorldLocation, 1, GL_FALSE, mWorldMatrix.data());
	...
	void DiffuseLightingDemo::CreateVertexBuffer(const Mesh& mesh, GLuint& vertexBuffer)
	{
		...
		for (size_t i = 0; i < sourceVertices.size(); i++)
		{
			...
			*/
			const Vector3f& position = sourceVertices.at(i);
			const Vector2f& uv = Vector2f(textureCoordinates->at(i).x(), textureCoordinates->at(i).y());
			const Vector3f& normal = normals.at(i);
			vertices.push_back(VertexPositionTextureNormal(Vector4f(position.x(), position.y(), position.z(), 1.0f), uv, normal));
		}
