...
namespace Rendering
{
	...
	WrappingModesDemo::WrappingModesDemo(Game& game, Camera& camera) :
		...
		mActiveWrappingMode(WrappingMode::Repeat), mKeyboardHandler(nullptr), mWorldMatrix(Eigen::Matrix4f::Identity())
	{
	}
	...
	void WrappingModesDemo::Initialize()
	{
		...
		VertexPositionTexture vertices[] =
		{
			...
			*/
			VertexPositionTexture(Vector4f(-halfSize, 1.0f, 0.0, 1.0f), Vector2f(0.0f, 3.0f)),
			VertexPositionTexture(Vector4f(-halfSize, size + 1.0f, 0.0f, 1.0f), Vector2f(0.0f, 0.0f)),
			VertexPositionTexture(Vector4f(halfSize, size + 1.0f, 0.0f, 1.0f), Vector2f(3.0f, 0.0f)),
			VertexPositionTexture(Vector4f(halfSize, 1.0f, 0.0f, 1.0f), Vector2f(3.0f, 3.0f))
		};
