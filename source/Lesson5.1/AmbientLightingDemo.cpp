...
namespace Rendering
{
	...
	AmbientLightingDemo::AmbientLightingDemo(Game& game, Camera& camera) :
		...
		mIndexCount(0), mColorTexture(0), mWorldMatrix(Eigen::Matrix4f::Identity())
	{
	}
	...
	void AmbientLightingDemo::Draw(const GameTime& gameTime)
	{
		...
		*/
		...
		glUniform4fv(mAmbientColorLocation, 1, mAmbientLight->Color().data());
	...
	void AmbientLightingDemo::CreateVertexBuffer(const Mesh& mesh, GLuint& vertexBuffer)
	{
		...
		for (size_t i = 0; i < sourceVertices.size(); i++)
		{
			...
			*/
			...
			vertices.push_back(VertexPositionTexture(Vector4f(position.x(), position.y(), position.z(), 1.0f), uv));
		}
