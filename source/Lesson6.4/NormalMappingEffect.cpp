...
namespace Rendering
{
	...
	void NormalMappingEffect::CreateVertexBuffer(const Mesh& mesh, GLuint& vertexBuffer) const
	{
		...
		const vector<Vector3f>& tangents = mesh.Tangents();
		assert(tangents.size() == sourceVertices.size());

		//const vector<vec3>& binormals = mesh.BiNormals();
		const vector<Vector3f>& binormals = mesh.BiNormals();
		...
		for (size_t i = 0; i < sourceVertices.size(); i++)
		{
			...
			*/
			...
			const Vector3f& tangent = tangents.at(i);
			const Vector3f& binormal = binormals.at(i);
			vertices.push_back(VertexPositionTextureNormalTangentBinormal(Vector4f(position.x(), position.y(), position.z(), 1.0f), uv, normal, tangent, binormal));
		}
