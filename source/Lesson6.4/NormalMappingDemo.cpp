...
namespace Rendering
{
	...
	const Vector2f NormalMappingDemo::LightRotationRate = Vector2f(2.0f * M_PI, 2.0f * M_PI);
	...
	NormalMappingDemo::NormalMappingDemo(Game& game, Camera& camera) :
		...
		mNormalMap(0), mTrilinearSampler(0), mShowNormalMapping(true), mKeyboardHandler(nullptr), mWorldMatrix(Eigen::Matrix4f::Identity())
	{
	}
	...
	void NormalMappingDemo::Initialize()
	{
		...
		VertexPositionTextureNormalTangentBinormal normalMappingVertices[] =
		{
			...
			*/
			VertexPositionTextureNormalTangentBinormal(Vector4f(-1.0f, 0.1f, 0.0f, 1.0f), Vector2f(0.0f, 1.0f), Vector3Helper::Backward, Vector3Helper::Down, Vector3Helper::Right),
			VertexPositionTextureNormalTangentBinormal(Vector4f(-1.0f, 2.1f, 0.0f, 1.0f), Vector2f(0.0f, 0.0f), Vector3Helper::Backward, Vector3Helper::Down, Vector3Helper::Right),
			VertexPositionTextureNormalTangentBinormal(Vector4f(1.0f, 2.1f, 0.0f, 1.0f), Vector2f(1.0f, 0.0f), Vector3Helper::Backward, Vector3Helper::Down, Vector3Helper::Right),
			VertexPositionTextureNormalTangentBinormal(Vector4f(1.0f, 0.1f, 0.0f, 1.0f), Vector2f(1.0f, 1.0f), Vector3Helper::Backward, Vector3Helper::Down, Vector3Helper::Right)
		};
		...
		VertexPositionTextureNormal fogVertices[] =
		{
			...
			*/
			VertexPositionTextureNormal(Vector4f(-1.0f, 0.1f, 0.0f, 1.0f), Vector2f(0.0f, 1.0f), Vector3Helper::Backward),
			VertexPositionTextureNormal(Vector4f(-1.0f, 2.1f, 0.0f, 1.0f), Vector2f(0.0f, 0.0f), Vector3Helper::Backward),
			VertexPositionTextureNormal(Vector4f(1.0f, 2.1f, 0.0f, 1.0f), Vector2f(1.0f, 0.0f), Vector3Helper::Backward),
			VertexPositionTextureNormal(Vector4f(1.0f, 0.1f, 0.0f, 1.0f), Vector2f(1.0f, 1.0f), Vector3Helper::Backward)
		};
		...
		mProxyModel->ApplyRotation(Affine3f(AngleAxisf(M_PI_2, Vector3Helper::Up)).matrix());

		//mWorldMatrix = scale(mat4(), vec3(5.0f));
		mWorldMatrix = Affine3f(Eigen::Scaling(5.0f)).matrix();
	...
	void NormalMappingDemo::UpdateAmbientLight(const GameTime& gameTime)
	{
		...
		static float ambientIntensity = mAmbientLight->Color().x();
		...
		if (glfwGetKey(mGame->Window(), GLFW_KEY_PAGE_UP) && ambientIntensity < 1.0f)
		{
			...
			mAmbientLight->SetColor(Vector4f(ambientIntensity, ambientIntensity, ambientIntensity, 1.0f));
		}
	...
	void NormalMappingDemo::UpdateDirectionalLight(const GameTime& gameTime)
	{
		...
		if (glfwGetKey(mGame->Window(), GLFW_KEY_HOME) && directionalIntensity < 1.0f)
		{
			...
			mDirectionalLight->SetColor(Vector4f(directionalIntensity, directionalIntensity, directionalIntensity, 1.0f));
		}
		...
		Affine3f lightRotationMatrix = Affine3f::Identity();
		if (rotationAmount.x() != 0)
		{
			//lightRotationMatrix = rotate(mat4(), rotationAmount.x, Vector3Helper::Up);
			lightRotationMatrix = Affine3f(AngleAxisf(rotationAmount.x(), Vector3Helper::Up));
		}

		if (rotationAmount.y() != 0)
		{
			//lightRotationMatrix = rotate(lightRotationMatrix, rotationAmount.y, mSpotLight->Right());
			lightRotationMatrix = lightRotationMatrix * Affine3f(AngleAxisf(rotationAmount.y(), mDirectionalLight->Right()));
		}

		if (rotationAmount.x() != 0.0f || rotationAmount.y() != 0.0f)
		{
			mDirectionalLight->ApplyRotation(lightRotationMatrix.matrix());
			mProxyModel->ApplyRotation(lightRotationMatrix.matrix());
		}
	}
	...
	void NormalMappingDemo::UpdateSpecularLight(const GameTime& gameTime)
	{
		...
		if (glfwGetKey(mGame->Window(), GLFW_KEY_INSERT) && specularIntensity < 1.0f)
		{
			...
			mSpecularColor = (Vector4f(specularIntensity, specularIntensity, specularIntensity, 1.0f));
		}
